...
- `rbt` 为 `in progress`：存在未跟踪实现，未接入 CMake，无正式测试。
...
| `CMap` / `RBT` | `aidb_rbt` + `aidb_map` | `rbt` **in progress**；`map` **planned** | 有序树、比较回调、map 建于树之上 | 统一按值存储/回调契约、allocator、status/out；未完成代码不能进入正式映射 | 旧 tree nil-node、key/value 深拷贝和 iterator 生命周期与新设计冲突 | 旧插入/查找/删除顺序作为参考；新项目必须有树不变量、随机序列、OOM 测试 |
